# --- main.py v7.1 (Syntax Fix) ---
# This version fixes the SyntaxError in the /help command.
...
async def handle_forward_task(user_bot, bot_client, task):
    ...
    for msg_id in range(start_id, end_id + 1):
        ...
        if time.time() - last_update > 2:
            ...
            except Exception: # Ignore errors if message is not modified
                pass
            last_update = time.time()
...
async def help_handler(event):
    # FIX: Using triple quotes to prevent SyntaxError
...
async def main():
    ...
    if OWNER_ID:
        # We start the bot client before sending a message with it
...
if __name__ == "__main__":
    ...
    # This was the source of the conflict. The bot client must be run as part of the main loop.
    with bot:
